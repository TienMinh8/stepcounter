...
public class MainActivity extends AppCompatActivity {
    ...
    private void initializeUI() {
        ...
        viewModel.getStepCount().observe(this, steps -> {
            safeSetText(binding.textSteps, steps != null ? String.valueOf(steps) : "0");
        });
        
        viewModel.getDistance().observe(this, distance -> {
            safeSetText(binding.textDistance, 
                    distance != null ? String.format(Locale.getDefault(), "%.2f km", distance) : "0.00 km");
        });
        
        viewModel.getCalories().observe(this, calories -> {
            safeSetText(binding.textCalories, 
                    calories != null ? String.format(Locale.getDefault(), "%d kcal", calories) : "0 kcal");
        });
        
        // Weekly statistics
        viewModel.getTotalWeeklySteps().observe(this, steps -> {
            if (steps != null) {
                safeSetText(binding.textWeeklySteps, "Total Steps: " + steps);
            } else {
                safeSetText(binding.textWeeklySteps, "Total Steps: 0");
            }
        });
        
        viewModel.getTotalWeeklyDistance().observe(this, distance -> {
            if (distance != null) {
                safeSetText(binding.textWeeklyDistance, 
                        String.format(Locale.getDefault(), "Total Distance: %.2f km", distance));
            } else {
                safeSetText(binding.textWeeklyDistance, "Total Distance: 0.00 km");
            }
        });
        
        viewModel.getTotalWeeklyCalories().observe(this, calories -> {
            if (calories != null) {
                safeSetText(binding.textWeeklyCalories, "Total Calories: " + calories + " kcal");
            } else {
                safeSetText(binding.textWeeklyCalories, "Total Calories: 0 kcal");
            }
        });
    ...
    private void updateChart(List<StepEntity> steps) {
        ...
        for (int i = 0; i < 7; i++) {
            String dayLabel = "";
            try {
                dayLabel = dayFormat.format(calendar.getTime());
            } catch (Exception e) {
            ...
            if (dayLabel != null) {
                ...
                labels.add("");  // Thêm nhãn trống nếu null
            }
            
            // Find step data for this day
            int stepsForDay = 0;
            for (StepEntity stepEntity : steps) {
                if (stepEntity == null || stepEntity.getDate() == null) {
                    continue;
                }
        ...
        BarDataSet dataSet = new BarDataSet(entries, entries.isEmpty() ? "" : "Daily Steps");
        try {
            dataSet.setColor(Color.parseColor("#4CAF50"));
        } catch (IllegalArgumentException e) {
            dataSet.setColor(Color.GREEN); // Fallback color
        }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_ACTIVITY_RECOGNITION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                String message = getString(R.string.permission_required);
                if (message == null) {
                    message = "Permission required to count steps";
                }
    ...
    /**
     * Helper method to safely set text on TextView
     */
